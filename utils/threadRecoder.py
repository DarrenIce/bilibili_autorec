...
class threadRecorder():
    def __init__(self):
        ...
        Log().debug_logger.info('threadRecoder 初始化完成')
    ...
    def add(self,tname,func,args,daemon):
        with self._lock:
            ...
            Log().debug_logger.info('[%s:%s]线程已启动，当前调起线程数: %s' % (a.native_id,tname, threading.active_count()))
            ...
            with self._lock:
                for pid in self.threads:
                    if not self.threads[pid][1].is_alive():
                        need_del.append(pid)
                for pid in need_del:
                    Log().debug_logger.info('[%s:%s]已结束' % (pid,self.threads[pid][0]))
                    del self.threads[pid]
                op_lst = ['[%s:%s]' % (pid,self.threads[pid][0]) for pid in self.threads]
                Log().debug_logger.info('当前调起线程数: %s, Details: %s' % (threading.active_count(), ', '.join(op_lst)))