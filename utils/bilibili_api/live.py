...
def get_room_play_info(room_display_id: int, verify: utils.Verify = None, cookies = None):
    """
    获取房间信息（真实房间号，封禁情况等）
    ...
    if cookies is None:
        ...
        resp = utils.get(api["url"], {"room_id": room_display_id}, cookies=cookies)
    return resp


def get_room_play_url(room_real_id: int, verify: utils.Verify = None, cookies = None):
    """
    获取获取房间直播流地址
    ...
    if cookies is None:
        ...
        resp = utils.get(api["url"], params, cookies=cookies)
    return resp
